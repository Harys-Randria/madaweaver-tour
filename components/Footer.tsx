      ...
      <div className="container-x relative z-10 grid gap-10 py-16 md:grid-cols-[1.4fr_1fr_1fr] md:gap-14">
        ...
        <div className="max-w-sm">
          ...
          <div className="mt-5 flex gap-2.5">
            {SOCIALS.map((s) => {
              const url = settings.social[s.name];
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5 text-sm text-cream/70">
            <li>
              <a href={`mailto:${settings.contact.email}`} className="hover:text-baobab-light">
                {settings.contact.email}
              </a>
            </li>
            <li>{settings.contact.phoneDisplay}</li>
            <li className="text-cream/55">{settings.contact.address}</li>
          </ul>
        </div>
